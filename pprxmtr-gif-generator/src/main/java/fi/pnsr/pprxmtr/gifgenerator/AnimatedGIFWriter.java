...
public class AnimatedGIFWriter {
	public static class GIFFrame {
		...
		public GIFFrame(BufferedImage frame, int delay, int disposalMethod, int transparentColor) {
			this(frame, 0, 0, delay, disposalMethod, USER_INPUT_NONE, TRANSPARENCY_INDEX_SET, transparentColor);
		}

		public GIFFrame(BufferedImage frame, int leftPosition, int topPosition, int delay, int disposalMethod) {
			this(frame, leftPosition, topPosition, delay, disposalMethod, USER_INPUT_NONE, TRANSPARENCY_INDEX_NONE,
					TRANSPARENCY_COLOR_NONE);
		}

		public GIFFrame(BufferedImage frame, int leftPosition, int topPosition, int delay, int disposalMethod,
				int userInputFlag, int transparencyFlag, int transparentColor) {
			if (frame == null) {
	 ...
	 *
	 * Based on QuadraticProbingHashTable.java
	 * <p>
	...
	private static class IntHashtable<E> {
		...
		private void rehash() {
			...
			// Copy table over
			for (HashEntry<E> element : oldArray) {
	...
	private static class InverseColorMap {
		...
		public void createInverseMap(int no_of_colors, int[] colorPalette) {
			...
			for (int i = 0; i < no_of_colors; i++) {
				...
				/**
				 * We start from the origin (0,0,0) of the quantized colors, calculate the distance between the cell
				 * center of the quantized colors and the current color map entry as follows: (rcenter * x + x/2) - red,
				 * where rcenter is the center of the Quantized red color map entry which is 0 since we start from 0.
				 */
		...
		public int getNearestColorIndex(int red, int green, int blue) {
			return invColorMap[red >> bitsDiscarded << (bitsReserved << 1) | green >> bitsDiscarded << bitsReserved
					| blue >> bitsDiscarded] & 0xff;
		}
	}

	/**
	 * Java port of C Implementation of Wu's Color Quantizer (v. 2) (see Graphics Gems vol. II, pp. 126-133) Author:
	 * Xiaolin Wu Dept. of Computer Science Univ. of Western Ontario London, Ontario N6A 5B7 wu@csd.uwo.ca
	 *
	 * Algorithm: Greedy orthogonal bipartition of RGB space for variance minimization aided by inclusion-exclusion
	 * tricks. For speed no nearest neighbor search is done. Slightly better performance can be expected by more
	 * sophisticated but more expensive versions.
	 *
	 * The author thanks Tom Lane at Tom_Lane@G.GP.CS.CMU.EDU for much of additional documentation and a cure to a
	 * previous bug.
	 *
	...
	private static class WuQuant {
		...
		/*
		 * Histogram is in elements 1..HISTSIZE along each axis, element 0 is for base or marginal value NB: these must
		 * start out 0!
		 */
		...
		/*
		 * At conclusion of the histogram step, we can interpret wt[r][g][b] = sum over voxel of P(c) mr[r][g][b] = sum
		 * over voxel of r*P(c) , similarly for mg, mb m2[r][g][b] = sum over voxel of c^2*P(c) Actually each of these
		 * should be divided by 'size' to give the usual interpretation of P() as ranging from 0 to 1, but we needn't do
		 * that here.
		 */

		/*
		 * We now convert histogram into moments so that we can rapidly calculate the sums of the above quantities over
		 * any desired box.
		 */
		...
		/*
		 * The next two routines allow a slightly more efficient calculation of Vol() for a proposed subbox of a given
		 * box. The sum of Top() and Bottom() is the Vol() of a subbox split in the given direction and with the
		 * specified new upper bound.
		 */

		/*
		 * We want to minimize the sum of the variances of two subboxes. The sum(c^2) terms can be ignored since their
		 * sum over both subboxes is the same (the sum for the whole box) no matter where we split. The remaining terms
		 * have a minus sign in the variance formula, so we drop the minus sign and MAXIMIZE the sum of the two terms.
		 */
		private float Maximize(Box cube, int dir, int first, int last, int cut[], long whole_r, long whole_g,
	...
	private static void dither_FloydSteinberg(int[] rgbTriplet, int width, int height, byte[] newPixels,
			int no_of_color, int[] colorPalette, int transparent_index) {
	 ...
	 *
	 * TYPE_INT_RGB = 1 TYPE_INT_ARGB = 2 TYPE_INT_ARGB_PRE = 3 TYPE_INT_BGR = 4 TYPE_3BYTE_BGR = 5 TYPE_4BYTE_ABGR = 6
	 * TYPE_4BYTE_ABGR_PRE = 7 TYPE_BYTE_GRAY = 10 TYPE_BYTE_BINARY = 12 TYPE_BYTE_INDEXED = 13 TYPE_USHORT_GRAY = 11
	 * TYPE_USHORT_565_RGB = 8 TYPE_USHORT_555_RGB = 9 TYPE_CUSTOM = 0.
	 *
	 * @param image
	 *            input BufferedImage
		...
		case DataBuffer.TYPE_BYTE:
			...
			if (type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_BYTE_BINARY) {
				...
				for (int i = 0; i < mapSize; i++) {
					palette[i] = (alphas[i] & 0xff) << 24 | (reds[i] & 0xff) << 16 | (greens[i] & 0xff) << 8
							| blues[i] & 0xff;
				}
				for (int i = 0; i < imageSize; i++) {
				...
				for (int i = 0, index = 0; i < imageSize; i++) {
					rgbs[i] = (bpixels[index++] & 0xff) << 16 | (bpixels[index++] & 0xff) << 8 | bpixels[index++] & 0xff
							| (bpixels[index++] & 0xff) << 24;
				}
			} else if (type == BufferedImage.TYPE_3BYTE_BGR) {
				for (int i = 0, index = 0; i < imageSize; i++) {
					rgbs[i] = 0xff000000 | (bpixels[index++] & 0xff) << 16 | (bpixels[index++] & 0xff) << 8
							| bpixels[index++] & 0xff;
				}
	...
	private static int[] reduceColorsDiffusionDither(int[] rgbTriplets, int width, int height, int colorDepth,
			byte[] newPixels, final int[] colorPalette) {
	...
	/**
	 * A child is made up of a parent(or prefix) code plus a suffix color and siblings are strings with a common
	 * parent(or prefix) and different suffix colors
	 */
	 ...
	 *
	 * @param os
	 *            OutputStream for the animated GIF
	 * @throws Exception
	 */
	 ...
	 * @param os
	 *            OutputStream for the animated GIF
	 * @param logicalScreenWidth
	 *            width of the logical screen. If it is less than or equal zero, it will be determined from the first
	 *            frame
	 * @param logicalScreenHeight
	 *            height of the logical screen. If it is less than or equal zero, it will be determined from the first
	 *            frame
	 * @throws Exception
	 */
	 ...
	 * @param delays
	 *            delays in millisecond for each frame
	 * @param os
	 *            OutputStream for the animated GIF
	 * @throws Exception
	 */
	...
	public void writeAnimatedGIF(GIFFrame[] frames, OutputStream os) throws Exception {
		...
		for (GIFFrame frame : frames) {
			...
			int[] pixels = getRGB(frame.getFrame());
	...
	private void writeFrame(int[] pixels, int imageWidth, int imageHeight, int imageLeftPosition, int imageTopPosition,
		...
		if (colorInfo[0] > 0x08) {
			bitsPerPixel = 8;
			if (isApplyDither) {
				colorInfo = reduceColorsDiffusionDither(pixels, imageWidth, imageHeight, bitsPerPixel, newPixels,
						colorPalette);
			} else {
	...
	public void writeFrame(OutputStream os, BufferedImage frame, int delay) throws Exception {
		...
		if (imageWidth > logicalScreenWidth) {
			imageWidth = logicalScreenWidth;
		}
		if (imageHeight > logicalScreenHeight) {
			imageHeight = logicalScreenHeight;
		}
	...
	public void writeFrame(OutputStream os, GIFFrame frame) throws Exception {
		...
		if (frameLeft >= logicalScreenWidth || frameTop >= logicalScreenHeight) {
			return;
		}
		if (frameLeft + imageWidth > logicalScreenWidth) {
			imageWidth = logicalScreenWidth - frameLeft;
		}
		if (frameTop + imageHeight > logicalScreenHeight) {
			imageHeight = logicalScreenHeight - frameTop;
		}
		...
		writeFrame(pixels, imageWidth, imageHeight, frame.getLeftPosition(), frame.getTopPosition(), frame.getDelay(),
				frame.getDisposalMethod(), frame.getUserInputFlag(), os);
	}

	// Unit of delay is supposed to be in millisecond
	private void writeGraphicControlBlock(OutputStream os, int delay, int transparent_color, int disposalMethod,
			int userInputFlag) throws Exception {
		// Scale delay
	...
	private void writeImageDescriptor(OutputStream os, int imageWidth, int imageHeight, int imageLeftPosition,
			int imageTopPosition, int colorTableSize) throws Exception {
	...
	// Write logical screen descriptor
	private void writeLSD(OutputStream os, short screen_width, short screen_height, short flags, byte bgcolor,
			byte aspectRatio) throws IOException {
